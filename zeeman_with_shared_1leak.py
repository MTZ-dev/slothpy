...
def hemholtz_energy_3d(filename: str, group: str, states_cutoff: int, fields: np.ndarray, spherical_grid: int, temperatures: np.ndarray, num_cpu: int, num_threads: int, internal_energy: bool = False) -> np.ndarray:
    ...
    theta_shared = multiprocessing.shared_memory.SharedMemory(create=True, size=theta.nbytes)
    phi_shared = multiprocessing.shared_memory.SharedMemory(create=True, size=phi.nbytes)
    temperatures_shared = multiprocessing.shared_memory.SharedMemory(create=True, size=temperatures.nbytes)
    magnetic_moment_shared = multiprocessing.shared_memory.SharedMemory(create=True, size=magnetic_moment.nbytes)
    soc_energies_shared = multiprocessing.shared_memory.SharedMemory(create=True, size=soc_energies.nbytes)

    # Copy data to shared memory
    theta_shared_arr = np.ndarray(theta.shape, dtype=theta.dtype, buffer=theta_shared.buf)
    phi_shared_arr = np.ndarray(phi.shape, dtype=phi.dtype, buffer=phi_shared.buf)
    temperatures_shared_arr = np.ndarray(temperatures.shape, dtype=temperatures.dtype, buffer=temperatures_shared.buf)
    magnetic_moment_shared_arr = np.ndarray(magnetic_moment.shape, dtype=magnetic_moment.dtype, buffer=magnetic_moment_shared.buf)
    soc_energies_shared_arr = np.ndarray(soc_energies.shape, dtype=soc_energies.dtype, buffer=soc_energies_shared.buf)
    ...
    for field_index, field in enumerate(fields):
        ...
        with threadpoolctl.threadpool_limits(limits=num_threads, user_api='blas'):
            with threadpoolctl.threadpool_limits(limits=num_threads, user_api='openmp'):
                ...
                with multiprocessing.Pool(num_process) as p:
                    eth = p.map(calculate_hemholtz_energyt_wrapper, arg_iter_hemholtz_energy_3d(magnetic_moment_shared_arr, soc_energies_shared_arr, field, theta_shared_arr, phi_shared_arr, temperatures_shared_arr, internal_energy))
