...
class Compound:
                    ...
                    if data_type == "susceptibility":
                        try:
                            ...
                            with h5py.File(self._hdf5, "r") as file:
                                description = (
                                    "ChiT dependance on direction,"
                                    f" B={self[f'{group}_3d_susceptibility', f'{group}_fields'][i_constant]} T"
                                )
                            T = True
                        except:
                            x = self[
                                f"{group}_3d_susceptibility", f"{group}_chi_3d"
                            ][0, i_constant, temp, :, :]
                            y = self[
                                f"{group}_3d_susceptibility", f"{group}_chi_3d"
                            ][1, i_constant, temp, :, :]
                        ...
                        x = self[
                            f"{group}_3d_hemholtz_energy", f"{group}_energy_3d"
                        ][0, i_constant, temp, :, :]
                        y = self[
                            f"{group}_3d_hemholtz_energy", f"{group}_energy_3d"
                        ][1, i_constant, temp, :, :]
                        ...
                        with h5py.File(self._hdf5, "r") as file:
                            description = (
                                "Energy dependence on direction,"
                                f" B={self[f'{group}_3d_hemholtz_energy', f'{group}_fields'][i_constant]} T"
                            )
                    elif data_type == "magnetisation":
                        x = self[
                            f"{group}_3d_magnetisation", f"{group}_mag_3d"
                        ][0, i_constant, temp, :, :]
                        y = self[
                            f"{group}_3d_magnetisation", f"{group}_mag_3d"
                        ][1, i_constant, temp, :, :]
                        ...
                        with h5py.File(self._hdf5, "r") as file:
                            description = (
                                "Magnetisation dependence on direction,"
                                f" B={self[f'{group}_3d_magnetisation', f'{group}_fields'][i_constant]} T"
                            )
                    title = description
                    max_array = np.array([np.max(x), np.max(y), np.max(z)])
                    lim = np.max(max_array)
                    ...
                    if data_type == "susceptibility":
                        try:
                            ...
                            with h5py.File(self._hdf5, "r") as file:
                                description = (
                                    "ChiT dependance on direction,"
                                    f" T={self[f'{group}_3d_susceptibility', f'{group}_temperatures'][i_constant]} K"
                                )
                            T = True
                        except:
                            x = self[
                        ...
                        with h5py.File(self._hdf5, "r") as file:
                            description = (
                                "Energy dependence on direction,"
                                f" T={self[f'{group}_3d_hemholtz_energy', f'{group}_temperatures'][i_constant]} K"
                            )
                    elif data_type == "magnetisation":
                        x = self[
                            f"{group}_3d_magnetisation", f"{group}_mag_3d"
                        ...
                        with h5py.File(self._hdf5, "r") as file:
                            description = (
                                "Magnetisation dependence on direction,"
                                f" T={self[f'{group}_3d_magnetisation', f'{group}_temperatures'][i_constant]} K"
                            )
                    title = description
                    max_array = np.array([np.max(x), np.max(y), np.max(z)])
                    lim = np.max(max_array)
    ...
    def animate_energy_3d(
        ...
        writer = PillowWriter(fps=fps)
        xo, yo, zo = self.calculate_hemholtz_energy_3d(
            group,
            states_cutoff,
            field,
            spherical_grid,
