...
class Rotation:
    ...
    def from_matrix_to_angles(matrix: "np.ndarray") -> "np.ndarray":
        ...
        return np.array([
            np.arctan2(matrix[1][2], matrix[0][2]),
    ...
    def from_angles_to_matrix(angles: "np.ndarray") -> "np.ndarray":
        return np.array([
            [-np.sin(angles[0]) * np.sin(angles[2]) + np.cos(angles[0]) * np.cos(angles[1]) * np.cos(angles[2]),
             -np.sin(angles[0]) * np.cos(angles[2]) - np.sin(angles[2]) * np.cos(angles[0]) * np.cos(angles[1]),
             np.sin(angles[1]) * np.cos(angles[0])],
            [np.sin(angles[0]) * np.cos(angles[1]) * np.cos(angles[2]) + np.sin(angles[2]) * np.cos(angles[0]),
             -np.sin(angles[0]) * np.sin(angles[2]) * np.cos(angles[1]) + np.cos(angles[0]) * np.cos(angles[2]),
             np.sin(angles[0]) * np.sin(angles[1])],
            [-np.sin(angles[1]) * np.cos(angles[2]), np.sin(angles[1]) * np.sin(angles[2]), np.cos(angles[1])]
        ])

    @staticmethod
    @jit(nopython=True, cache=True)
    def from_angles_to_axis_angle(angles: "np.ndarray") -> "np.ndarray":
