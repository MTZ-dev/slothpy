...
class Rotation:
    ...
    def from_matrix_to_axis_angle(matrix: "np.ndarray") -> "np.ndarray":
        ...
        axis_angle[3] = np.arccos(np.divide(matrix[2][2] - np.power(np.cos(beta_sph),2), np.power(np.sin(beta_sph),2)))
        return axis_angle
    ...
    #     alpha_sph = np.arctan2(axis_angle[1], axis_angle[0])
    #     beta_sph = np.arccos(axis_angle[2])
        
    #     cos_alpha_sph = np.cos(alpha_sph)
    #     sin_alpha_sph = np.sin(alpha_sph)
    #     cos_beta_sph = np.cos(beta_sph)
    #     sin_beta_sph = np.sin(beta_sph)
    #     cos_theta = np.cos(axis_angle[3])
    #     sin_theta = np.sin(axis_angle[3])
        
    #     matrix_sph = np.empty((3, 3), dtype=np.float64)
    #     matrix_sph[0, 0] = cos_alpha_sph * cos_beta_sph
    #     matrix_sph[0, 1] = -sin_alpha_sph
    #     matrix_sph[0, 2] = sin_beta_sph * cos_alpha_sph
    #     matrix_sph[1, 0] = sin_alpha_sph * cos_beta_sph
    #     matrix_sph[1, 1] = cos_alpha_sph
    #     matrix_sph[1, 2] = sin_alpha_sph * sin_beta_sph
    #     matrix_sph[2, 0] = -sin_beta_sph
    #     matrix_sph[2, 1] = 0.0
    #     matrix_sph[2, 2] = cos_beta_sph

    #     matrix = np.empty((3, 3), dtype=np.float64)
    #     matrix[0, 0] = cos_theta * matrix_sph[0, 0] - sin_theta * matrix_sph[1, 0]
    #     matrix[0, 1] = cos_theta * matrix_sph[0, 1] - sin_theta * matrix_sph[1, 1]
    #     matrix[0, 2] = cos_theta * matrix_sph[0, 2] - sin_theta * matrix_sph[1, 2]
    #     matrix[1, 0] = sin_theta * matrix_sph[0, 0] + cos_theta * matrix_sph[1, 0]
    #     matrix[1, 1] = sin_theta * matrix_sph[0, 1] + cos_theta * matrix_sph[1, 1]
    #     matrix[1, 2] = sin_theta * matrix_sph[0, 2] + cos_theta * matrix_sph[1, 2]
    #     matrix[2, 0] = matrix_sph[2, 0]
    #     matrix[2, 1] = matrix_sph[2, 1]
    #     matrix[2, 2] = matrix_sph[2, 2]
    ...
    @jit(nopython=True, cache=False)
    def from_axis_angle_to_matrix(axis_angle: np.ndarray) -> np.ndarray:
        axis_angle = axis_angle.astype(np.float64)
        axis = axis_angle[:3] / np.linalg.norm(axis_angle[:3])
        angle = axis_angle[3]

        alpha_sph = np.arctan2(axis[1], axis[0])
        beta_sph = np.arccos(axis[2])

        matrix_sph = np.array([
            [np.cos(alpha_sph) * np.cos(beta_sph), -np.sin(alpha_sph), np.sin(beta_sph) * np.cos(alpha_sph)],
            [np.sin(alpha_sph) * np.cos(beta_sph), np.cos(alpha_sph), np.sin(alpha_sph) * np.sin(beta_sph)],
            [-np.sin(beta_sph), 0, np.cos(beta_sph)]
        ], dtype=np.float64)
        ...
        matrix = matrix_sph @ rotation_matrix @ matrix_sph.T
    ...
    #def from_axis_angle_to_matrix(axis_angle: "np.NDArray") -> "np.NDArray":
        ...
        #matrix[2][1] = c
        #matrix[3][3] = 1
        #matrix = np.array([[c, -s, 0], [s, c, 0], [0, 0, 1]]) #nie działa, błąd, czemu inne działają?
        #matrix = matrix_sph@matrix@matrix_sph.T
        #return matrix.T
...
if __name__ == "__main__":
    ...
    def test():
        ...
        test4 = Rotation.from_matrix(test2.matrix)

        # print("test4")
        # print(test4.angles)
        # print(test4.matrix)
        # print(test4.axis_angle)
