...
class Compound:
    ...
    def interactive_plot_3d(self, group: str, data_type: str, colour_map_name='dark_rainbow_r',
                            ...
                            bar=True, bar_scale=False, temp_rounding=2, field_rounding=2, axis_off=False):
        ...
        if bar:
            c = colour1[temp_i]
            axins = ax.inset_axes([-0.05, .7, .098, .2])
            axins.bar(1, indicator1[temp_i], width=0.2, color=c)
            axins.set_ylim(0, 1)
            c = colour2[field_i]
            axins2 = ax.inset_axes([-0.05, .2, .098, .2])
            ...
            if bar_scale:
                ...
                axins2.yaxis.set_major_formatter(matplotlib.ticker.FuncFormatter(my_ticks2))
