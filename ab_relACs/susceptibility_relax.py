...
from constants import H_BAR, KB, M_AU, S_TIME_PS, H
...
def dawsn(typingctx, x_ty):
    ...
    def codegen(ctx: BaseContext, builder: IRBuilder, signature, args):
        ...
        if x_ty == types.float32 and "f32" in _DAWSN_ALIASES:
            fnty = llir.FunctionType(llir.FloatType(), [llir.FloatType(), i32])
            callee = cgutils.get_or_insert_function(
                builder.module, fnty, name=_DAWSN_ALIASES["f32"]
            )
            return builder.call(callee, [xval, zero])

        fnty = llir.FunctionType(llir.DoubleType(), [llir.DoubleType(), i32])
        callee = cgutils.get_or_insert_function(
            builder.module, fnty, name=_DAWSN_ALIASES.get("f64", "pybridge_dawsn_d")
        )

        x_as_f64 = (
            xval if x_ty == types.float64 else builder.fpext(xval, llir.DoubleType())
        )
...
def add_KR_bundle(out, Yb_table, Jhat_p_table, Jhat_m_table, q_0, weight):
    J = Jhat_p_table.shape[0]
    N = Jhat_p_table.shape[1]
            ...
            for b in range(N):
                ...
                for e in range(N):
                    ...
                    if k != 0.0:
                        ...
                        if b == a:
                            trace[thread_id, t_index] += (
                                coeff * k * rho_aa * Y_j[a, e, e, a]
                            )
...
def get_Y_q(
    ...
    for j in range(normal_modes.shape[1]):
        for i in range(dof_array.shape[0]):
            dof = dof_array[i]
            Y_q[j] += (
                (1 / np.sqrt(freq[j] / H_CM_1))
                * H_grad[i]
                * normal_modes[dof[0], j]
                * masses_inv_sqrt[dof[0]]
                * 1
                / np.sqrt(M_AU)
                * number_of_kpoints_inv_sqrt
                * np.exp(
                    -2j
                    * np.pi
                    * (k_point[0] * dof[1] + k_point[1] * dof[2] + k_point[2] * dof[3])
                )
            )
...
@njit(nogil=True, cache=True, fastmath=True, inline="never")
def _build_dynamical_matrix(
    hessian: np.ndarray, masses_inv_sqrt: np.ndarray, kpoint: np.ndarray
):
    ...
    for nx in range(hessian.shape[0]):
        for ny in range(hessian.shape[1]):
            for nz in range(hessian.shape[2]):
                dyn_mat += hessian[nx, ny, nz, :, :] * np.exp(
                    2j * pi * (kpoint[0] * nx + kpoint[1] * ny + kpoint[2] * nz)
                )
...
def frequencies_eigenvectors(dynamical_matrix):
    ...
    return np.where(
        frequencies_squared >= 0,
        np.sqrt(np.abs(frequencies_squared)),
        -np.sqrt(np.abs(frequencies_squared)),
    ), eigenvectors
            ...
            for c in range(N):
                for d in range(N):
                    ...
                    G = (
                        n1
                        * (n2 + 1.0)
                        * gaussian(w_n[a, c] - f1 + f2, lw_total, cutoff)
                    )
                    val += (
                    ...
                    if a == c:
                        for k in range(N):
                            G = (
                                n1
                                * (n2 + 1.0)
                                * gaussian(w_n[k, d] - f1 + f2, lw_total, cutoff)
                            )
                            val -= (
                                0.5
                                * (
                    ...
                    if b == d:
                        for k in range(N):
                            G = (
                                n1
                                * (n2 + 1.0)
                                * gaussian(w_n[k, c] - f1 + f2, lw_total, cutoff)
                            )
                            val -= (
                                0.5
                                * (
...
@overload(
    hilbert, nogil=True, fastmath=True, cache=True, inline="never", prefer_literal=True
)
...
def ov_hilbert_symm(E, d, kind):
    if isinstance(kind, types.Literal):
        if kind.literal_value == 1:

            def impl(E, d, kind):
                return gauss_hilbert_symm(E, d)

            return impl
        elif kind.literal_value == 0:

            def impl(E, d, kind):
                return lorentz_hilbert_symm(E, d)
...
def Jhat_p_symm(w_ab, wq, n_q, d, cutoff, kind):
    if w_ab > 0 and np.abs(w_ab - wq) < cutoff:
        return hilbert_symm(w_ab - wq, d, kind) * n_q
    if w_ab < 0 and np.abs(w_ab + wq) < cutoff:
        return hilbert_symm(w_ab + wq, d, kind) * (n_q + 1)
    return 0.0 + 0.0 * 1j
...
def Jcorr_symm(w_cd, w_ab, wq, n_q, d, beta, cutoff, kind):
    ...
    if u < 0 and np.abs(u + wq) < cutoff:
        return hilbert_symm(u + wq, d, kind) * n_q * zeta(w_ab + wq, beta)
    if u > 0 and np.abs(u - wq) < cutoff:
        return hilbert_symm(u - wq, d, kind) * (n_q + 1) * zeta(w_ab - wq, beta)
    return 0.0 + 0.0 * 1j
...
@njit(nogil=True, cache=True, fastmath=True, inline="never")
def add_PSI_bundle(
    out, A, Yb_table, wb, nb, delta, beta, w_n, q_0, cutoff, weight, kind, symm
):
    N = w_n.shape[0]
    J = wb.size
    coeff = H_BAR * 0.5 * weight
...
def add_KR_bundle_comp(
    M_KR_i_t, Yb_table, Jhat_p_table, Jhat_m_table, q_0, weight, run_KR
):
    raise NotImplementedError
    ...
    if isinstance(run_KR, types.Literal):
        if run_KR.literal_value == 0:
            ...
            def impl(
                ...
                return add_KR_bundle(
                    M_KR_i_t, Yb_table, Jhat_p_table, Jhat_m_table, q_0, weight
                )

            return impl


def add_PSI_bundle_comp(
    M_PSI_i_t,
    A_e,
    Yb_table,
    wb,
...
def add_R21_bundle_comp(
    R21_i_t, Yb_table, w_n, Jhat_p_table, q_0, degeneracy_tolerance, weight, run_R21
):
    raise NotImplementedError
...
def gauss_hilbert(E, d):
    ...
    return (
        ...
        * np.sqrt(np.pi * 0.5)
        / d
        * (
            np.exp(-(factor_sqrt * factor_sqrt))
            + 2j / np.sqrt(np.pi) * dawsn_f(factor_sqrt)
        )
    )


@njit(nogil=True, cache=True, fastmath=True, inline="always")
def lorentz_hilbert_symm(E, d):
    return 1j / (E + 1j * d)


@njit(nogil=True, cache=True, fastmath=True, inline="always")
def gauss_hilbert_symm(E, d):
    factor_sqrt = E / (np.sqrt(2) * d)
    return (
        np.sqrt(np.pi * 0.5)
        / d
...
def build_matrices_no_R41(
    ...
    for i in range(2, arr.size, 2):
        w_n_direct_max = min(w_n_direct_max, arr[i])
    ...
    for i in prange(grid.shape[0]):
        ...
        freq, modes = frequencies_eigenvectors(
            _build_dynamical_matrix(hessian, masses_inv_sqrt_outer, q)
        )
        freq *= AU_BOHR_CM_1
        ...
        fwhm_j = (
            gamma_fwhm * np.ones_like(wb)
        )  # TODO: can implement ab inito model for different wb and T (move into the loop) - adaptive_fwhm in config
        ...
        for t_index in range(beta.shape[0]):
            ...
            Jhat_p_table, Jhat_m_table = build_Jp_Jm_tables_j(
                w_n, wb, bose, fwhm_j, cutoff_j, kind, symm
            )
            if run_R21:
                if symm:
                    Jhat_p_symm_table = Jhat_p_table
                else:
                    Jhat_p_symm_table = build_Jp_symm_table_j(
                        w_n, wb, bose, fwhm_j, cutoff_j, kind
                    )
...
def build_matrices_R41(
    ...
    masses_inv_sqrt_outer = np.outer(masses_inv_sqrt, masses_inv_sqrt)
    gamma = np.asarray([0.0, 0.0, 0.0])
    freq0, modes0 = frequencies_eigenvectors(
        _build_dynamical_matrix(hessian, masses_inv_sqrt_outer, gamma)
    )
    ...
    Yb_array = np.zeros(
        (threads_number, max_grid_size, H_grad.shape[1], H_grad.shape[2]), np.complex128
    )
    ...
    for i in prange(grid.shape[0]):
        ...
        if not q_0:
            Yb_array[
                thread_id, raman_counter_wb[thread_id] : raman_counter_2wb[thread_id]
            ] = np.conjugate(np.transpose(Yb, (0, 2, 1)))
            wb_array[
                thread_id, raman_counter_wb[thread_id] : raman_counter_2wb[thread_id]
            ] = wb
...
def solve_susceptibility(
    ...
    if normalize:
        ...
        chi_T[:, :] = (
            np.sign(chi_T[:, 0].real)[:, np.newaxis] * chi_T[:, :].real
            + 1j * np.sign(chi_T[:, 0].imag)[:, np.newaxis] * chi_T[:, :].imag
        )
...
def plot_spinphonon_weighted_phonon_dos(
    ...
    If save_path is provided, a CSV with the plotted numerical data is also saved
    alongside the figure using the same stem:
        figure: /path/to/name.png
        data  : /path/to/name_data.csv
    """
    ...
    if all_freq.size == 0:
        raise RuntimeError(
            "No modes found in the requested [modes_low, modes_high] window."
        )
    ...
    if convolution is None:
        ...
        freq_range = np.linspace(
            fmin_plot, fmax_plot, int(resolution), dtype=np.float64
        )
    ...
    if save_path is not None:
        ...
        csv_path = save_path_obj.with_name(f"{save_path_obj.stem}_data.csv")

        if convolution is None:
            data = np.column_stack(
                (
                    bin_edges[:-1],  # bin_left_cm^-1
                    bin_edges[1:],  # bin_right_cm^-1
                    centers,  # bin_center_cm^-1
                    hist,  # raw histogram
                    hist_norm,  # plotted histogram
                )
            )
            header = (
                "bin_left_cm^-1,"
            ...
            data = np.column_stack(
                (
                    ...
                    hist,  # raw histogram
                    hist_norm,  # plotted histogram
                    freq_range,  # convolution x
                    conv,  # raw convolution
                    conv_norm,  # plotted convolution
                )
            )
            header = (
                "bin_left_cm^-1,"
        ...
        if energy_lines is not None:
            if isinstance(energy_lines, tuple):
                for x0 in energy_lines[0]:
                    ax.axvline(
                        x=float(x0), color="olivedrab", lw=1.4, alpha=1.0, zorder=10000
                    )
                for x0 in energy_lines[1]:
                    ax.axvline(
                        x=float(x0), color="sienna", lw=1.1, alpha=1.0, zorder=10000
                    )
            else:
...
def _make_weight_reducer(weight_mode: str, E: np.ndarray, temperature: float | None):
    ...
    if wm == "thermal_offdiag":
        ...
        def reduce_W(absY2):
            diag = np.diagonal(absY2, axis1=1, axis2=2)  # (Jm, N)
            return (absY2 * P_usym[None, :, :]).sum(axis=(1, 2)) - (diag * p).sum(
                axis=1
            )
    ...
    raise ValueError(
        "weight_mode must be one of: fro, fro_offdiag, thermal_sym, thermal_offdiag"
    )
...
def _gather_spinphonon_weighted_dos_parallel(
    ...
    nq = grid.shape[0]
    ndof = masses_inv_sqrt.shape[
        0
    ]  # number of phonon modes == dynamical matrix dimension
