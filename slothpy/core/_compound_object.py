...
class Compound:
    ...
    def animate_3d(self, group: str, data_type: str, animation_variable: str, i_start=0, i_end=2, i_constant=0,
        ...
        if animation_variable == 'temperatures':
            temps = temps[i_start:i_end]
        elif animation_variable == 'fields':
            fields = fields[i_start:i_end]
        ...
        with writer.saving(fig, f'{filename}.gif', dpi):
            if animation_variable == 'temperature':
                for temp in range(temps.shape[i_constant]):
                    T = False
                    if data_type == 'chiT':
                        x = self[f'{group}_3d_susceptibility', f'{group}_chit_3d'][0, i_constant, temp, :, :]
                        y = self[f'{group}_3d_susceptibility', f'{group}_chit_3d'][1, i_constant, temp, :, :]
                        z = self[f'{group}_3d_susceptibility', f'{group}_chit_3d'][2, i_constant, temp, :, :]
                        description = f"ChiT dependance on direction, B={self[f'{group}_3d_susceptibility', f'{group}_fields'][i_constant]} T"
                        T = True
                    if data_type == 'chi':
                        x = self[f'{group}_3d_susceptibility', f'{group}_chi_3d'][0, i_constant, temp, :, :]
                        y = self[f'{group}_3d_susceptibility', f'{group}_chi_3d'][1, i_constant, temp, :, :]
                        z = self[f'{group}_3d_susceptibility', f'{group}_chi_3d'][2, i_constant, temp, :, :]
                        description = f"Chi dependance on direction, B={self[f'{group}_3d_susceptibility', f'{group}_fields'][i_constant]} T"
                    elif data_type == 'hemholtz_energy':
                        x = self[f'{group}_3d_hemholtz_energy', f'{group}_energy_3d'][0, i_constant, temp, :, :]
                        y = self[f'{group}_3d_hemholtz_energy', f'{group}_energy_3d'][1, i_constant, temp, :, :]
                        z = self[f'{group}_3d_hemholtz_energy', f'{group}_energy_3d'][2, i_constant, temp, :, :]
                        description = f"Energy dependence on direction, B={self[f'{group}_3d_hemholtz_energy', f'{group}_fields'][i_constant]} T"
                    elif data_type == 'magnetisation':
                        x = self[f'{group}_3d_magnetisation', f'{group}_mag_3d'][0, i_constant, temp, :, :]
                        y = self[f'{group}_3d_magnetisation', f'{group}_mag_3d'][1, i_constant, temp, :, :]
                        z = self[f'{group}_3d_magnetisation', f'{group}_mag_3d'][2, i_constant, temp, :, :]
                        description = f"Magnetisation dependence on direction, B={self[f'{group}_3d_magnetisation', f'{group}_fields'][i_constant]} T"
                    title = description
                    max_array = np.array([np.max(x), np.max(y), np.max(z)])
                    lim = np.max(max_array)
                    norm = plt.Normalize(z.min(), z.max())
                ...
                for field in range(fields.shape[i_constant]):
                    T = False
                    if data_type == 'chiT':
                        x = self[f'{group}_3d_susceptibility', f'{group}_chit_3d'][0, field, i_constant, :, :]
                        y = self[f'{group}_3d_susceptibility', f'{group}_chit_3d'][1, field, i_constant, :, :]
                        z = self[f'{group}_3d_susceptibility', f'{group}_chit_3d'][2, field, i_constant, :, :]
                        description = f"ChiT dependance on direction, T={self[f'{group}_3d_susceptibility', f'{group}_temperatures'][i_constant]} K"
                        T = True
                    elif data_type == 'chi':
                        x = self[f'{group}_3d_susceptibility', f'{group}_chi_3d'][0, field, i_constant, :, :]
                        y = self[f'{group}_3d_susceptibility', f'{group}_chi_3d'][1, field, i_constant, :, :]
                        z = self[f'{group}_3d_susceptibility', f'{group}_chi_3d'][2, field, i_constant, :, :]
                        description = f"Chi dependance on direction, T={self[f'{group}_3d_susceptibility', f'{group}_temperatures'][i_constant]} K"
                    elif data_type == 'hemholtz_energy':
                        x = self[f'{group}_3d_hemholtz_energy', f'{group}_energy_3d'][0, field, i_constant, :, :]
                        y = self[f'{group}_3d_hemholtz_energy', f'{group}_energy_3d'][1, field, i_constant, :, :]
                        z = self[f'{group}_3d_hemholtz_energy', f'{group}_energy_3d'][2, field, i_constant, :, :]
                        description = f"Energy dependence on direction, T={self[f'{group}_3d_hemholtz_energy', f'{group}_temperatures'][i_constant]} K"
                    elif data_type == 'magnetisation':
                        x = self[f'{group}_3d_magnetisation', f'{group}_mag_3d'][0, field, i_constant, :, :]
                        y = self[f'{group}_3d_magnetisation', f'{group}_mag_3d'][1, field, i_constant, :, :]
                        z = self[f'{group}_3d_magnetisation', f'{group}_mag_3d'][2, field, i_constant, :, :]
                        with h5py.File(self._hdf5, 'r') as file:
                            description = f"Magnetisation dependence on direction, T={self[f'{group}_3d_magnetisation', f'{group}_temperatures'][i_constant]} K"
                    title = description
                    max_array = np.array([np.max(x), np.max(y), np.max(z)])
                    lim = np.max(max_array)
                    norm = plt.Normalize(z.min(), z.max())
