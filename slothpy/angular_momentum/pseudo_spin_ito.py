...
def set_condon_shortley_phases_for_matrix_in_z_pseudo_spin_basis(
...
def get_soc_matrix_in_z_magnetic_momentum_basis(
    filename, group, start_state, stop_state, rotation=None
):
    (
        magnetic_momenta,
        soc_energies,
    ...
    soc_matrix = set_condon_shortley_phases_for_matrix_in_z_pseudo_spin_basis(
        magnetic_momenta, soc_matrix
    )

    return soc_matrix


def get_soc_matrix_in_z_total_angular_momentum_basis(
    filename, group, start_state, stop_state, rotation=None
):
    (
    ...
    soc_matrix = set_condon_shortley_phases_for_matrix_in_z_pseudo_spin_basis(
        total_angular_momenta, soc_matrix
    )

    return soc_matrix


def get_zeeman_matrix_in_z_magnetic_momentum_basis(
    filename, group, field, orientation, start_state, stop_state, rotation=None
):
    (
        magnetic_momenta,
        soc_energies,
    ...
    zeeman_matrix = (
        set_condon_shortley_phases_for_matrix_in_z_pseudo_spin_basis(
...
def get_zeeman_matrix_in_z_total_angular_momentum_basis(
    filename, group, field, orientation, start_state, stop_state, rotation=None
):
    (
        total_angular_momenta,
    ...
    zeeman_matrix = hermitian_x_in_basis_of_hermitian_y(
        zeeman_matrix, total_angular_momenta[2, :, :]
    )
    zeeman_matrix = (
        set_condon_shortley_phases_for_matrix_in_z_pseudo_spin_basis(
            total_angular_momenta, zeeman_matrix
        )
    )
...
def get_decomposition_in_z_magnetic_momentum_basis(
    filename, group, start_state, stop_state, rotation=None
):
    soc_matrix = get_soc_matrix_in_z_magnetic_momentum_basis(
        filename, group, start_state, stop_state, rotation
    )
...
def get_decomposition_in_z_total_angular_momentum_basis(
    filename, group, start_state, stop_state, rotation=None
):
...
def matrix_from_ito_real(J, coefficients):
    ...
    for i in coefficients:
        ...
        if q > 0:
            matrix += (
                0.5
                * (ito_matrix(J, k, -q) + ((-1) ** q) * ito_matrix(J, k, q))
                * i[2]
            )
        if q == 0:
            matrix += ito_matrix(J, k, q) * i[2]
