...
class Compound:
    ...
    def __new__(cls, *args, **kwargs):
        raise TypeError(
            "The Compound object should not be instantiated directly. Use a"
            " Compound creation function instead."
        )
    ...
    def __setitem__(self, key, value) -> None:
        ...
        if isinstance(key, str):
            try:
                ...
                error_message = str(e)
                raise Exception(
                    f"Error encountered while trying to set dataset {key} in"
            ...
            try:
                with h5py.File(self._hdf5, "r+") as file:
                    ...
                    new_dataset = new_group.create_dataset(
                        key[1], shape=value.shape, dtype=value.dtype
                    )
                    new_dataset[:] = value[:]
                ...
                error_message = str(e)
                raise Exception(
                    f'Error encountered while trying to set group "{key[0]}"'
                    f' and dataset "{key[1]}" in .slt file: {self._hdf5}:'
    ...
    def calculate_g_tensor_and_axes_doublet(
        self, group: str, doublets: np.ndarray, slt: str = None
    ):
        try:
            (
                g_tensor_list,
    ...
    def soc_energies_cm_1(
        ...
        try:
            ...
            error_message = str(e)
            raise Exception(
                "Error encountered while trying to get SOC energies from"
                f" file: {self._hdf5} - group {group}: {error_type}:"
                f" {error_message}"
            )
            ...
            try:
                with h5py.File(self._hdf5, "r+") as file:
                    ...
                    soc_energies_dataset.attrs["Description"] = (
                        "Dataset containing SOC (Spin-Orbit Coupling)"
                        f" energies calculated from group: {group}."
                    )
    ...
    def total_angular_momenta_matrix(
        ...
        if (not isinstance(states_cutoff, np.int)) or (states_cutoff < 0):
            raise ValueError(
                f"Invalid states cutoff, set it to positive integer or 0 for"
                f" all states."
            )
    ...
    def decomposition_in_z_magnetic_momentum_basis(
        ...
        if (not isinstance(stop_state, int)) or (stop_state < 0):
            raise ValueError(
                f"Invalid states number, set it to positive integer or 0 for"
                f" all states."
            )
    ...
    def soc_crystal_field_parameters(
        ...
        if order > 2 * dim:
            raise ValueError(
                f"Order of ITO parameters exeeds 2S. Set it less or equal."
            )
        ...
        if (
            ...
            try:
                ...
                try:
                    ...
                    raise Exception(
                        "Failed to form matrix from ITO parameters.\n"
                        " Error(s) encountered while trying compute the"
                        f" matrix: {error_print_2}, {error_print_3}"
                    )
    ...
    def soc_zeem_in_angular_magnetic_momentum_basis(
        ...
        if (matrix_type not in ["zeeman", "soc"]) or (
            ...
            raise ValueError(
                f'Only valid matrix_type are "soc" or "zeeman" and basis_type'
                f' are "angular" or "magnetic"'
            )
        ...
        if matrix_type == "zeeman" and (
            ...
            raise ValueError(
                f"For Zeeman matrix provide filed value and orientation."
            )
            ...
            try:
                with h5py.File(self._hdf5, "r+") as file:
                    ...
                    mag_3d_dataset = new_group.create_dataset(
                        f"{slt}_mag_3d",
                        shape=(3, x.shape[1], x.shape[2]),
                        dtype=np.float64,
                    )
                    mag_3d_dataset.attrs["Description"] = (
                        "Dataset containing 3D magnetisation as meshgird"
                        " (0-x,1-y,2-z) arrays over sphere (T:"
                        f" {temperature} K, H: {field} T) calculated from"
                        f" group: {group}."
                    )
            ...
            try:
                with h5py.File(self._hdf5, "r+") as file:
                    ...
                    chit_3d_dataset.attrs["Description"] = (
                        ...
                        " meshgird (0-x,1-y,2-z) arrays over sphere (T:"
                        f" {temperature} K, H: {field} T) calculated from"
                        f" group: {group}."
                    )
                ...
                error_message = str(e)
                raise Exception(
                    "Error encountered while trying to save 3D magneic"
    ...
    def plot_chit_3d(
        ...
        try:
            ...
            with h5py.File(self._hdf5, "r") as file:
                description = file[f"{group}_3d_susceptibility"][
                    f"{group}_chit_3d"
                ].attrs["Description"]
            T = True
        except:
        ...
        title = (
            "S"
            + description[
                description.index("sphere")
                + 1 : description.index("calculated")
                - 1
            ]
        )
        if show:
            ...
            if save:
                if axis_off:
                    fig.savefig(
                        f"{group}_3d_chit.tiff", transparent=True, dpi=600
                    )
    ...
    def plot_mag_3d(
        ...
        show=True,
        save=False,
        colour_map_name="light_rainbow_l",
        lim_scalar=1,
        ticks=0,
        r_density=0,
        c_density=0,
        ...
        with h5py.File(self._hdf5, "r") as file:
            description = file[f"{group}_3d_magnetisation"][
                f"{group}_mag_3d"
            ].attrs["Description"]
        title = (
            "S"
        ...
        if show:
            ...
            ax.grid(False)
            ax.set_xlabel(
                r"$M\ /\ \mathrm{\mu_{B}}$", labelpad=10 * len(str(ticks)) / 4
            )
            ax.set_ylabel(
                r"$M\ /\ \mathrm{\mu_{B}}$", labelpad=10 * len(str(ticks)) / 4
            )
            ...
            if save:
                if axis_off:
                    fig.savefig(
                        f"{group}_3d_mag.tiff", transparent=True, dpi=600
                    )
                else:
    ...
    def table_energy_and_g(
        ...
        if decomp == "angular":
            composition_frac_matrix = self[
                f"{group}_total_angular_decomposition",
                f"{group}_magnetic_momenta_matrix",
            ]
            composition_states = self[
                f"{group}_total_angular_decomposition",
        ...
        if krames:
            ...
            table.cell(row, 0).paragraphs[0].add_run(
                ") of"
                f" {number_of_doublets}{' ground' if first_level == 0 else ''} Kramers"
                " doublets"
            )
            table.cell(row, 0).merge(table.cell(row, 4))
            row += 1
            ...
            for index in range(number_of_doublets):
                table.cell(row, 0).paragraphs[0].add_run(
                    f"{int(g_tensors[first_level + index][0]) + 1}."
                )
                table.cell(row, 1).paragraphs[0].add_run(
                    f"{energies[(first_level + index) * 2]:.3f}"
                )
                table.cell(row, 2).paragraphs[0].add_run(
                    f"{g_tensors[first_level + index][1]:.4f}"
                )
                table.cell(row, 3).paragraphs[0].add_run(
                    f"{g_tensors[first_level + index][2]:.4f}"
                )
                table.cell(row, 4).paragraphs[0].add_run(
                    f"{g_tensors[first_level + index][3]:.4f}"
                )
                row += 1
            ...
            table.cell(row, 0).paragraphs[0].add_run(
                ") of"
                f" {number_of_doublets}{' ground' if first_level == 0 else ''} Ising"
                " doublets"
            )
            table.cell(row, 0).merge(table.cell(row, 3))
            row += 1
            ...
            for index in range(number_of_doublets):
                ...
                table.cell(row, 1).paragraphs[0].add_run(
                    f"{energies[(first_level + 1 + index * 2)]:.3f}"
                )
                table.cell(row, 2).paragraphs[0].add_run(
        ...
        for index in range(number_to_decompose):
            ...
            for inner_index in range(len(composition_states)):
                if (
                    composition_frac_matrix[first_composition + index * 2][
                        inner_index
                    ]
                    <= threshold
                ):
    ...
    def animate_mag_3d(
        ...
        field: np.ndarray,
        spherical_grid: int,
        temperature_start: np.float64,
        temperature_stop: np.float64,
        frames: int,
        num_cpu: int,
        colour_map_name="dark_rainbow",
        lim_scalar=1,
        ticks=1,
        axis_off=False,
        r_density=0,
        ...
        xo, yo, zo = self.calculate_mag_3d(
            group, states_cutoff, field, spherical_grid, temps, num_cpu
        )
