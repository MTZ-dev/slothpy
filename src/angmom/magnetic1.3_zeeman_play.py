...
def get_soc_moment_energies_from_hdf5_orca(path: str, hdf5_file: str, states_cutoff: int) -> tuple[np.ndarray[np.complex128], np.ndarray[np.float64]]:
    ...
    bohr_magneton = 4.66864374e-1# Bohr magneton in a.u./T
    ...
    soc_energies = (soc_energies[:states_cutoff] - soc_energies[0]) * 219474.6
...
def calculate_partition_function(energies: np.ndarray[np.float64], temperature: np.float64) -> np.float64:
    ...
    """
    kB = 0.69503877  # Boltzmann constant a.u./K
...
def mth(path: str, hdf5_file: str, states_cutoff: int, fields: np.ndarray[np.float64], grid: np.ndarray[np.float64], temperatures: np.ndarray[np.float64], num_cpu: int) -> np.ndarray[np.float64]:
    ...
    fields = np.ascontiguousarray(fields/1.5)
    ...
    for i in range(fields.shape[0]):
        mth_array[:,i] = mt[i][0]
        zeeman_list.append(mt[i][1])

    return mth_array / bohr_magneton, zeeman_list #returning values in Bohr magnetons
...
if __name__ == '__main__':
    ...
    # c = 1/4.3597482E-18 / avogadro 
    # print(f"c = {c}")

    fields = np.linspace(0.00001, 7, 70)
    temperatures1 = np.linspace(1.8, 1.8, 1)
    temperatures2 = np.linspace(1, 300, 300)
    grid = np.array([[0.,0.,1.,1.]])#np.loadtxt('grid.txt', usecols = (1,2,3,4))
    ...
    mth5, zeeman_list = mth('.', 'DyCo_cif.hdf5', 16, fields, grid, temperatures1, 2)
